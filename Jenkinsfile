...
podTemplate(
    ...
    node(label) {
        ...
        stage('Run kubectl') {
            container('kubectl') {
                sh "kubectl replace -f rest-sample-app-deployment.yaml -n ${nameSpace}"
                sh "kubectl replace -f rest-sample-app-service.yaml -n ${nameSpace}"
                sh "kubectl replace -f rest-sample-app-ingress.yaml -n ${nameSpace}"
            }
        }
    }
}